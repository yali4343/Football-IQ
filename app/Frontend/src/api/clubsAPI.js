...
export async function getClubDescription(clubId, signal) {
  const response = await fetch(`${CLUBS_URL}/${clubId}/description`, {
    signal,
  });

  if (!response.ok) {
    throw new Error(`Failed to fetch club description: ${response.status}`);
  }
